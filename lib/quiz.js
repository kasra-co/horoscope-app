...
var HoroscopeQuiz = module.exports =  React.createClass({
	...
	getResultIndex: function () {
		...
		var date = new Date();
		var daysSinceEpoch = Math.floor( date.getTime() / 86400000 );
		var uniqueIndex = ( resultIndex + daysSinceEpoch ) % results.length;
		return uniqueIndex;
	...
	render: function() {
		...
		return (
			<div id="quiz" className={ "horoscopeQuiz " + ( result != null? "completed" : "" ) }>
				<QuestionList imagePath={ this.props.imagePath } selectOption={ this.selectOption } result={ result } questions={ this.props.quiz.questions }  />
				{ result != null? <Result imagePath={ this.props.imagePath } resultImageIndex={ this.state.resultImageIndex } result={ result } shortUrl={ this.props.shortUrl } siteUrl={ this.props.siteUrl } /> : null }
			</div>
		);
	}
